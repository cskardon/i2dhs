...
local_authority_plot <- df_local_authority_data |> 
    ...
    scale_y_continuous(limits = c(13.5,26), breaks = seq(13.5, 26, 0.5)) +
    labs(
        title = "Population prescribed drugs for anxiety,\ndepression and psychosis for each local authority in Scotland",
        x = "Year",
        y = "Percent of Population",
 ...
 local_authority_plot <- local_authority_plot +
    gghighlight(area_name %in% c('Midlothian', 'East Lothian', 'Clackmannanshire'),
              unhighlighted_params = list(linewidth = 0.8,
                                          colour = alpha("grey", 0.3),
                                          linetype = "dotted"))
local_authority_plot
```
...
Add in 'total' difference

```{r "Calculate the total difference"}
national_diff = max(df_national_data$percent_prescribed) - min(df_national_data$percent_prescribed)
```

```{r}
df_local_authority_change |> 
    ggplot( aes(x=reorder(area_name, diff_percent), y=diff_percent, fill=as.factor(area_name))) + 
...
```{r}
df_local_authority_change |> 
    mutate(above = diff_percent > national_diff, below = diff_percent < national_diff) |> 
    ggplot( aes(x=reorder(area_name, diff_percent), y=diff_percent, fill=factor(above, c(TRUE, FALSE)))) + 
    geom_bar( stat="identity" ) +
    coord_flip() +
     labs(
        title = "Difference between 2014 and 2023 in the percent of the population prescribed\ndrugs for anxiety, depression and psychosis by local authority in Scotland",
