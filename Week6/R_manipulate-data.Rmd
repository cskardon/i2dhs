...
There's no data dictionary to answer which is which, so just grouping / splitting by `sex`

```{r}
fev_data$gender <- ifelse(fev_data$sex == 0, "female", "male")

fev_gendered <- fev_data |> 
  group_by(sex) 

summarise(fev_gendered, n())
...
```{r}
# mean of FEV for smokers, mean of FEV for non smokers

fev_smokes <- fev_data |> 
  group_by(smoke)

summarise(fev_smokes, mean(fev), mean(age))

fev_smokes_gender <- fev_data |>
  group_by(smoke, sex)

summarise(fev_smokes_gender, mean(fev))
```
