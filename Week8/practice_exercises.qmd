...
admission_data <- read_csv("data/alcohol_related_admissions.csv") 
...
median_admissions %>%
  ...
  rename(alcohol_admissions = median_admissions) %>%
  ggplot(aes(x=reorder(health_board,alcohol_admissions), y=alcohol_admissions, fill=isGrampian)) + 
  geom_bar(stat = "identity") +
  coord_flip() + 
  xlab(NULL) + 
  ylab("Age-sex standardised rate per 100,000") + 
...
```{r}
median_admissions %>%
  filter(year %in% c(2010, 2015, 2019)) %>%   # only select these two years
  mutate(year = as.factor(year)) %>%          # convert years to categories ("factors")
  rename(alcohol_admissions = median_admissions) %>%
  ggplot(aes(x=health_board,
         y=alcohol_admissions,
         fill = year)) + #       alpha = 8)) + #why does this show on the legend??
  geom_bar(position="dodge", stat = "identity") +  # without "dodge", we get a stacked bar chart
  coord_flip() + 
  xlab(NULL) + 
  ylab("Age-sex standardised rate per 100,000") + 
...
deprivation_data <- read_csv("data/simd20_2019.csv") 
...
```{r}
admission_data_2019 <- admission_data |> 
  filter(year == 2019) |> 
  rename(`NHS health board` = health_board) |> 
  group_by(`NHS health board`) |> 
  summarise(area_total_admissions = sum(alcohol_admissions))

combined_admission_and_deprived_ratio <- merge(admission_data_2019, deprived_ratio, by = 'NHS health board')
```
...
```{r}
plot <- combined_admission_and_deprived_ratio |> 
  ggplot(aes(x = percent_deprived, y = area_total_admissions)) +       
  geom_point()+
  geom_smooth(se = FALSE) + 
  xlab("Deprived ratio")+                                                  # remove x-axis label
  ylab("Number of alcohol admissions") +              # label y-axis
  ggtitle("Alcohol-related hospital admissions by NHS health board in 2019") + # add title
  theme_bw() 
...
```{r}
plot <- combined_admission_and_deprived_ratio |> 
  ggplot(aes(y = percent_deprived, x = str_wrap(`NHS health board`, 10), size = area_total_admissions)) +       
  geom_point()+
    xlab("Deprived ratio")+                                                  # remove x-axis label
  ylab("Number of alcohol admissions") +              # label y-axis
  ggtitle("Alcohol-related hospital admissions by NHS health board in 2019") + # add title
